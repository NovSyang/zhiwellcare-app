...
<template><section class="center-guide"><template v-if="phase === 'guide'"><h2>确认自然中心位置</h2><p>请将手部自然放置在设备上，调整到舒适的中心位置。开始后请保持不动约 1 秒。</p><button class="button primary wide" @click="start">开始确认</button></template><template v-else-if="phase === 'calibrating'"><h2>请保持当前位置</h2><p>正在确认自然中心位置……</p><div class="calibration-progress"><span :style="{ width: `${Math.round(calibration.progress * 100)}%` }"></span></div></template><template v-else><h2>自然中心位置确认未完成</h2><p>请确认设备连接正常，并重新保持舒适的中心位置。</p><button class="button primary" @click="start">重新确认</button></template></section></template>
