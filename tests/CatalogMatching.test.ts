...
describe('目录源与目录服务（Mock）', () => {
  ...
  it('listModelGames 只返回姿态类游戏给不倒翁，力矩底座暂无可用游戏', async () => {
    ...
    expect(wobbleGames.map((game) => game.gameId).sort()).toEqual(['kart-racing', 'river-drift', 'target-reach', 'trajectory-follow'])
  ...
  it('CatalogService.getDeviceGameProfile 派生可用/不可用清单', async () => {
    ...
    // 桌面底座面对四款姿态游戏均缺失姿态传感标签。
    expect(profile!.unavailableGames).toHaveLength(4)
  ...
  it('listGamesWithModels 为每款游戏标注兼容设备名', async () => {
    ...
    expect(items.find((item) => item.gameId === 'kart-racing')?.durationPresetsMin).toEqual([])
  })
})
